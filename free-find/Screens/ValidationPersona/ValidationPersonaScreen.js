...
const ValidationPersona = (props) => {
    ...
    const createPersonaAccount = async (event) => {
        const response = await axios.post(`${config.REACT_APP_API_URL}/users/register`, {
            name: name,
            email: email,
            password: password,
            address: address,
            ...
            <View style={styles.tail}>
                ...
                <TouchableOpacity style={[styles.button, { backgroundColor: '#4F6C93' }]}
                    onPress={() => {
                        ...
                        if ((address !== "") && (city !== "") && (sex !== "") && (id_card.trim() !== "") && (birthdate.trim() !== "") && (nationality.trim() !== "")
                            && (education_level.trim() !== "") && (ability.trim() !== "") && (useDay.trim() !== "") && (useMonth.trim() !== "")
                            && (useYear.trim() !== "")
                        ) {
